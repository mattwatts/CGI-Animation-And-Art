"""
PROJECT: Logic Garden 383d (9-Core Physical Construct // 3x3 Rubik's Tensor)
FORMAT: YouTube Shorts (1080x1920)
METADATA: RUBIKS CUBE, MULTI-THREADING, EXACT HARDWARE, PHOTOREALISM
EXECUTION: 24.0s Sequence. True 3D Beveled Geometry. 9x Concurrency.
RULES ENFORCED: 
- 243 Unified Dual-Layer Cubies (Solid Plastic Base + Offset Stickers).
- Absolute Geometric Clearance: 0% overlap between 9 independent rendering queues.
...
def shade_color(hex_color, n_cam):
    # Studio lighting simulation locked purely to the camera axis
...
def create_cube_data(solver_seq):
    """Instantiates 27 blocks, scrambles symmetrically backwards."""
    blocks = []
    for cy in [-100, 0, 100]:
        for cx in [-100, 0, 100]:
            for cz in [-100, 0, 100]:
...
# ------------------------------------------------------------------
# 9 ASYMMETRIC SCRAMBLE / SOLVER MATRICES
# ------------------------------------------------------------------
SEQ_1 = [(1,100,1), (0,100,-1), (2,100,1), (1,100,-1), (2,-100,-1), (0,-100,1), (2,-100,1), (0,-100,-1), (1,-100,1), (2,100,-1), (1,-100,-1), (0,100,1)]
SEQ_2 = [(0,-100,1), (1,-100,1), (2,-100,1), (0,0,-1), (1,100,-1), (2,100,1), (0,100,-1), (1,0,1), (2,-100,-1), (0,-100,-1), (1,-100,-1), (2,0,1)]
SEQ_3 = [(2,100,-1), (1,0,1), (0,100,1), (2,0,-1), (1,-100,1), (0,-100,-1), (2,-100,1), (1,100,-1), (0,0,1), (2,100,1), (1,-100,-1), (0,100,-1)]
SEQ_4 = [(1,0,-1), (0,100,-1), (2,-100,-1), (1,-100,1), (0,0,1), (2,0,1), (1,100,1), (0,-100,-1), (2,100,-1), (1,-100,-1), (0,100,1), (2,-100,1)]
SEQ_5 = [(0,100,1), (2,-100,1), (1,100,1), (0,0,-1), (2,0,1), (1,-100,-1), (0,-100,1), (2,100,-1), (1,0,-1), (0,100,-1), (2,-100,-1), (1,100,-1)]
SEQ_6 = [(2,-100,-1), (1,100,1), (0,-100,1), (2,0,-1), (1,-100,-1), (0,100,1), (2,100,1), (1,0,1), (0,-100,-1), (2,-100,1), (1,100,-1), (0,100,-1)]
SEQ_7 = [(1,-100,1), (0,-100,-1), (2,100,-1), (1,0,1), (0,100,1), (2,0,1), (1,100,-1), (0,0,-1), (2,-100,1), (1,-100,-1), (0,-100,1), (2,100,1)]
SEQ_8 = [(0,-100,-1), (2,100,1), (1,-100,1), (0,100,-1), (2,-100,1), (1,100,1), (0,0,1), (2,0,-1), (1,-100,-1), (0,-100,1), (2,100,-1), (1,100,-1)]
SEQ_9 = [(2,100,1), (1,-100,-1), (0,100,-1), (2,-100,-1), (1,100,1), (0,-100,1), (2,0,1), (1,0,-1), (0,100,1), (2,100,-1), (1,-100,1), (0,-100,-1)]

# Instantiate the Master Thread Allocation (3x3 Cartesian Grid)
CUBES = []
def init_data():
    global CUBES
    # ABSOLUTE GEOMETRIC CLEARANCE: Pushing X offsets to 360 to prevent optical crowding
    # Y offsets: 440, 60, -320 (380 unit uniform spacing)
    CUBES.append({'blocks': create_cube_data(SEQ_1), 'seq': SEQ_1, 'ox': -360, 'oy': 440})
    CUBES.append({'blocks': create_cube_data(SEQ_2), 'seq': SEQ_2, 'ox':    0, 'oy': 440})
    CUBES.append({'blocks': create_cube_data(SEQ_3), 'seq': SEQ_3, 'ox':  360, 'oy': 440})
    
    CUBES.append({'blocks': create_cube_data(SEQ_4), 'seq': SEQ_4, 'ox': -360, 'oy':  60})
    CUBES.append({'blocks': create_cube_data(SEQ_5), 'seq': SEQ_5, 'ox':    0, 'oy':  60})
    CUBES.append({'blocks': create_cube_data(SEQ_6), 'seq': SEQ_6, 'ox':  360, 'oy':  60})
    
    CUBES.append({'blocks': create_cube_data(SEQ_7), 'seq': SEQ_7, 'ox': -360, 'oy': -320})
    CUBES.append({'blocks': create_cube_data(SEQ_8), 'seq': SEQ_8, 'ox':    0, 'oy': -320})
    CUBES.append({'blocks': create_cube_data(SEQ_9), 'seq': SEQ_9, 'ox':  360, 'oy': -320})
...
def render_frame(packet):
    ...
    # 1. OPTICS & RIGID CAMERA 
    pitch = np.radians(-28) 
    yaw = np.radians(-42) + (t * 0.12)
    sys_rot = get_R(0, pitch) @ get_R(1, yaw)
    
    # PULLED BACK WITH COMPRESSED FOCAL LENGTH FOR GEOMETRIC CLEARANCE
    cam_dist = 4500.0  
    focal = 3000.0 
    ...
    # 3. KINEMATIC DISPATCH OVER 9 INDEPENDENT MATRICES
    for cube in CUBES:
        render_queue = []
        seq = cube['seq']
            ...
            for poly in b['polys']:
                ...
                if n_cam[2] < 0.01:
                    ...
                    for pt in poly['v']:
                        ...
                        z = p_c[2] + cam_dist
                        if z > 10:
                            # 2D QUADRANT OFFSET APPLIED
                            pts_2d.append([p_c[0] * focal / z + cube['ox'], p_c[1] * focal / z + cube['oy']])
                            zs.append(z)
                    
                    if len(pts_2d) == 4:
                        render_queue.append({'z': np.mean(zs), 'poly': pts_2d, 'n_cam': n_cam, 'c': poly['c']})

        # O(1) Mathematically Faultless Depth Sort (Isolated per sector)
    ...
    ax.text(-460, 860, "LG-383d :: 9-CORE RUBIK'S 3X3 TENSOR", color=C_TEXT, fontsize=22, fontname='monospace', weight='bold', zorder=82)
    ...
    if phase_ratio < (T_START / DURATION):
        s1, c1 = "9X SCRAMBLED // ASYMMETRIC", C_F
        s2 = "0 / 12 PER CORE"
    ...
    ax.text(-460, -760, "9-CORE SYSTEM    :", color=C_TEXT, fontsize=16, fontname='monospace', weight='bold', zorder=82)
...
def run_batch():
    ...
    print(f"LG-383d: 9-CORE RUBIK'S 3x3 MATRIX (Clearance Active) [CORES: {cpu_cores}]")
    init_data() 
