"""
PROJECT: Logic Garden 417b (The Minor Planet Tensor // Seamless Loop Kinematics)
...
- TATHĀTĀ LOOP: Rotations locked to K-Multiplier (integer revolutions per 24s window).
- Internal Core fluid/node differential spin injected.
...
OUT_DIR = "frames_417b_dwarf_planets"
...
# K_Crust and K_Core represent exact integer revolutions per 24.0s for Seamless Looping.
# Speed: K=1 is slowest (15 deg/sec), K=8 is fastest (120 deg/sec).
# Structure: [True_Radius_KM, Scaled_Radius, Core_Radius, [Screen_X, Screen_Y], C_Core, C_Crust, C_Node, K_Crust, K_Core]
DATA_MATRIX = {
    'PLUTO':    [1188, 175.0, 115.0, [-340,  560], '#1E293B', '#B45309', '#FFB300', 3, 5],
    'ERIS':     [1163, 171.3, 110.0, [   0,  560], '#111115', '#E2E8F0', '#00D2FF', 1, 3],
    'MAKEMAKE': [ 715, 105.3,  70.0, [ 340,  560], '#1A202C', '#CC6600', '#FF3300', 4, 6],
    'CHARON':   [ 606,  89.2,  50.0, [-340,   50], '#111115', '#94A3B8', '#DE008A', 2, 4],
    'QUAOAR':   [ 555,  81.7,  45.0, [   0,   50], '#1E293B', '#64748B', '#00C853', 5, 7],
    'SEDNA':    [ 497,  73.1,  40.0, [ 340,   50], '#111115', '#DF0000', '#FFB300', 3, 5],
    'CERES':    [ 473,  69.6,  40.0, [-340, -460], '#020617', '#475569', '#E2E8F0', 8, 10],
    'ORCUS':    [ 458,  67.4,  38.0, [   0, -460], '#1E293B', '#005599', '#00D2FF', 6, 8],
    'SALACIA':  [ 423,  62.2,  35.0, [ 340, -460], '#111115', '#334155', '#94A3B8', 5, 7],
}

print(f"PHASE 1: 3X3 SEAMLESS KINEMATIC MATRICES COMPILED. Z-DEPTH SWAY NEUTRALISED.")
...
def seed_volume_nodes(n_pts, r_min, r_max):
...
for name, data in DATA_MATRIX.items():
    NODE_ARRAYS[name] = {
        'crust': seed_surface_nodes(N_CRUST, data[1]),
        'core': seed_volume_nodes(N_CORE, data[2] * 0.2, data[2] - 2.0)
    }
...
def render_frame(f_idx):
    ...
    def process_kinematic_nodes(dr, dphi, dtheta, v_spin, pos, h_col, size=3.5, expose_in_void=False):
        for i in range(len(dr)):
            cur_t = (dtheta[i] + np.radians(v_spin)) % (2 * np.pi)
            is_in_void = (cur_t > T_MAX) # Void defined strictly beyond 270 degrees
    ...
    for name, data in DATA_MATRIX.items():
        true_km, s_r, c_r, screen_pos, c_core, c_crust, c_node, k_crust, k_core = data
        world_p = M_inv @ np.array([screen_pos[0], screen_pos[1], 0.0])

        # 1. Compile Core Substrate
        ...
        # 2. Compile Mantle/Crust Interlock
        ...
        # 3. Seamless Loop Kinematics (v_spin = K * 15.0 to guarantee 360 loop inside 24s)
        crust_dr, crust_dphi, crust_dtheta = NODE_ARRAYS[name]['crust']
        core_dr, core_dphi, core_dtheta = NODE_ARRAYS[name]['core']
        
        # Crust (renders strictly ON crust, vanishes inside void)
        process_kinematic_nodes(crust_dr, crust_dphi, crust_dtheta, t_sec * (k_crust * 15.0), world_p, c_node, 3.5, expose_in_void=False)
        # Core (renders strictly INSIDE void, proving internal differential rotation)
        process_kinematic_nodes(core_dr, core_dphi, core_dtheta, t_sec * (k_core * 15.0), world_p, '#FFFFFF', 4.5, expose_in_void=True)
    ...
    ax.text(-500, 900, "LG-417b :: THE MINOR PLANET TENSORS", color=C_TEXT, fontsize=24, fontname='monospace', weight='bold', zorder=82)
    ax.text(-500, 850, "[SFI-1.00] SEAMLESS KINEMATICS & DIFFERENTIAL CORE ROTATION", color=C_GUI, fontsize=14, fontname='monospace', weight='bold', zorder=82)
...
def run_batch():
    ...
    print(f"LG-417b: MINOR PLANET TENSORS ENGAGED [CORES: {cpu_cores}]")
    with mp.Pool(processes=cpu_cores) as pool:
        for finished_frame in pool.imap_unordered(render_frame, range(TOTAL_FRAMES), chunksize=8):
            pass
    print("Compilation Complete. Tathata Loop Achieved. Nodes resolved.")
