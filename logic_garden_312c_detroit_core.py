"""
SOVEREIGN CODE: logic_garden_312c_detroit_core.py
FORMAT: YouTube Shorts (1080x1920)
SYSTEM: Python Multicore / O(1) Kinematic Phase Tensor
SCENE: LG-312c_v2 (Detroit Core / Dual-Platter Marshaling Tensor)
HOTFIX: ease_parabola Traceback Resolved, Emerald Reactor Secured
"""
...
OUT_DIR = "frames_312c_detroit_core"
os.makedirs(OUT_DIR, exist_ok=True)

# -------- HIGH-CONTRAST ENGINEERING PALETTE --------
C_BG        = '#FFFFFF'       # Daylight Protocol Baseline
C_DETROIT   = '#3E2723'       # Deep Melanin/Carbon-Copper Alloy
C_VIRIDIAN  = '#229954'       # Fractal Acoustic Dampener (Green Afro)
C_EMERALD   = '#2ECC71'       # Central Power Reactor / Glow
C_VOID      = '#0B0F19'       # Obsidian Optics / Record Vinyl
C_STEEL     = '#7F8C8D'       # Dual-Platter Decks / Substrate
C_IRON      = '#1C2833'       # Machinery Base / Stand
C_GOLD      = '#F39C12'       # Output Telemetry / Sparks
C_CYAN      = '#00FFFF'       # Dial Nodes
C_TEXT      = '#111111'
...
def ease_parabola(t):
    """Absolute smoothing derivative bridging 0.0 -> 1.0 -> 0.0"""
...
def render_frame(f):
    ...
    def draw_leg(hx, hy, kx, ky, fx, fy, zbase, is_fg):
        ax.plot([hx, kx], [hy, ky], color=C_DETROIT, lw=65, zorder=zbase, solid_capstyle='round', solid_joinstyle='round')
        ax.plot([kx, fx], [ky, fy], color=C_DETROIT, lw=50, zorder=zbase, solid_capstyle='round', solid_joinstyle='round')
        ax.add_patch(Circle((kx, ky), 20, facecolor=C_IRON, edgecolor=C_BG, lw=3, zorder=zbase+1))
        # Sleek stage boots
        ...
        if is_fg: ax.plot([fx-30, fx+40], [fy-5, fy-5], color=C_EMERALD, lw=6, zorder=zbase+2)

    draw_leg(CX_M+50, CY_hip, kx_R, ky_R, ax_R, ay_R, 10, False)
    draw_leg(CX_M-50, CY_hip, kx_L, ky_L, ax_L, ay_L, 12, True)
    
    # 3. THE KINEMATIC TORSO & EMERALD CHEST
    # -------------------------------------------------
    CY_neck = CY_hip + 420
    
    # Athletic, upright posture
    t_poly = [(CX_M-110, CY_hip-20), (CX_M+110, CY_hip-20), (CX_M+140, CY_neck), (CX_M-140, CY_neck)]
    ax.add_patch(Polygon(t_poly, facecolor=C_DETROIT, edgecolor=C_TEXT, lw=8, zorder=13, joinstyle='round'))
    
    # THE LITERAL GLOWING EMERALD
    # Pulsing with the 144 BPM system clock
    ...
    # 4. CRANIUM, SHADES, AND THE FRACTAL "AFRO"
    # -------------------------------------------------
    # Head bobs slightly deeper than the chest for that grooving vibe
    head_sway = np.sin((tau * beat_freq * 0.5) * 2 * np.pi) * 15.0
    cx_H, cy_H = CX_M + head_sway, CY_neck + 110 - y_bounce * 1.5
    
    # FRACTAL ACOUSTIC DAMPENER (The Green Afro)
    afro_rad = 160.0
    ax.add_patch(Circle((cx_H, cy_H+20), afro_rad, facecolor=C_VIRIDIAN, edgecolor=C_TEXT, lw=10, zorder=10))
    ax.add_patch(Circle((cx_H-20, cy_H+40), afro_rad*0.8, facecolor=C_BG, alpha=0.1, zorder=11))
    ax.add_patch(Circle((cx_H-40, cy_H+60), afro_rad*0.5, facecolor=C_BG, alpha=0.2, zorder=12))
    ...
    # Visor reflections (Emerald glint)
    ...
    # Happy/Upbeat logic output equivalent (Subtle Emerald grin/vent)
    sm_ang = np.linspace(np.radians(220), np.radians(320), 40)
    ax.plot(cx_H + np.cos(sm_ang)*45, cy_H - 10 + np.sin(sm_ang)*45, color=C_EMERALD, lw=6, solid_capstyle='round', zorder=41)
    ...
    scr_drill = np.sin(tau * 24.0 * np.pi) * 30.0 # Fast rhythmic 16th note scratching
    wr_L_x = plat_L_x + scr_drill
    wr_L_y = deck_y + deck_h/2 + 20
    el_L_x, el_L_y = solve_ik(L_sh_x, L_sh_y, wr_L_x, wr_L_y+20, 190, 200, 1) # Elbow points out
    
    ax.plot([L_sh_x, el_L_x, wr_L_x], [L_sh_y, el_L_y, wr_L_y+20], color=C_DETROIT, lw=40, zorder=35, solid_capstyle='round', solid_joinstyle='round')
    ax.add_patch(Circle((wr_L_x, wr_L_y+20), 25, facecolor=C_IRON, edgecolor=C_TEXT, lw=4, zorder=36))
    ...
    ax.plot([R_sh_x, el_R_x, wr_R_x], [R_sh_y, el_R_y, wr_R_y], color=C_DETROIT, lw=40, zorder=35, solid_capstyle='round', solid_joinstyle='round')
    ax.add_patch(Circle((wr_R_x, wr_R_y), 25, facecolor=C_IRON, edgecolor=C_TEXT, lw=4, zorder=36))
    ...
    txt_top = "LG-312c(v2): DETROIT CORE // DUAL-PLATTER MARSHALING TENSOR"
    ...
    sys_str = "AESTHETIC PARADIGM OVERRIDE: TENSION DELETED"
    sys_col = C_EMERALD
...
def run_batch():
    ...
    print(f"LG-312c(v2): DETROIT CORE TENSOR [CORES: {cpu_cores}]")
    print(f"Executing PROTOCOL: Continuous 10.0s Loop (600 Frames) // Traceback Resolved")

    with mp.Pool(processes=cpu_cores) as pool:
        frames = range(TOTAL_FRAMES)
        for finished_frame in pool.imap_unordered(render_frame, frames, chunksize=16):
            pass
    print("Compilation Complete. Sub-routines verified.")
