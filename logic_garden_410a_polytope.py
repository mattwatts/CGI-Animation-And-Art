"""
PROJECT: Logic Garden 410c (The Dimensional Polytope Tensor // Deep-Time Matrix)
FORMAT: YouTube Shorts (1080x1920)
METADATA: POLYTOPE, TESSERACT, 4D GEOMETRY, KINEMATICS, DISCRETE MATHS
EXECUTION: 40.0s Sequence. True 4D Mathematical Rotation mapped to 3D Physical Struts.
RULES ENFORCED:
...
- HOTFIX: 40-Second Deep-Time execution. Angular mapping completely flattened.
"""
...
# ======== SEQUENCE PARAMETERS ========
FPS = 60
DURATION = 40.0  # EXPANDED TIMELINE FOR GLACIAL STUDY
...
print(f"PHASE 2: DEEP-TIME STUDY MODE ENGAGED [DURATION: {DURATION}s]")
...
def render_frame(f_idx):
    ...
    # GLACIAL STUDY MODE: 0.5 degrees per second lock.
    cam_angle = t_sec * 0.5 - 45.0
    M_cam = rx(-15.0) @ ry(cam_angle)
    # Reduced distance for monumental framing while holding structure
    cam_dist = 1100.0 
    
    # 2. TIMELINE MATHEMATICS (Absolute 4D Deep-Time Sweep)
    T_HL_START  = 3.0
    T_HL_END    = 6.0
    T_ROT_START = 8.0
    T_ROT_END   = 36.0  # 28 entire seconds dedicated to the 4D inversion
    ...
    if t_sec > T_ROT_START:
        ...
        theta_xw = ease_in_out(rp) * np.pi 
        theta_yz = ease_in_out(rp) * (np.pi / 4.0)
    ...
    ax.text(-500, 880, "LG-410c :: POLYTOPE TENSOR (DEEP TIME)", color=C_TEXT, fontsize=22, fontname='monospace', weight='bold', zorder=82)
    ...
    if t_sec < T_HL_END:
        ...
        state_msg = "PHASE 2: THE 4-DIMENSIONAL TENSOR (INVERSION)"
...
def run_batch():
    ...
    print(f"LG-410c: DEEP-TIME POLYTOPE TENSOR ENGAGED [CORES: {cpu_cores}]")
