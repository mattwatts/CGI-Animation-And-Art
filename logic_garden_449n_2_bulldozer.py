"""
PROJECT: Logic Garden 449n (Exact Physical Construct // Heavy Bulldozer Matrix - HOTFIX)
...
- Exact structural integration: Figure-8 track crossover eradicated. True continuous track oval mapped.
...
def generate_dozer_vectors():
    ...
    for t_y in [-1.3, 1.3]:
        ...
        # Track loop mathematically constructed (Continuous Oval, NO Crossover)
        # Rear Sprocket Curve (Top to Bottom mapping CCW)
        th1 = np.linspace(np.pi/2, 3*np.pi/2, 12)
        rx = -1.6 + 0.5 * np.cos(th1)
        rz =  0.6 + 0.5 * np.sin(th1)
        
        # Bottom Straightaway Bridge
        bx = np.linspace(-1.6, 2.0, 15)
        bz = np.linspace(0.1, 0.1, 15)
        
        # Front Idler Curve (Bottom to Top mapping CCW)
        th2 = np.linspace(-np.pi/2, np.pi/2, 12)
        fx =  2.0 + 0.4 * np.cos(th2)
        fz =  0.5 + 0.4 * np.sin(th2)
        
        # Top Straightaway Bridge
        tx = np.linspace(2.0, -1.6, 15)
        tz = np.linspace(0.9, 1.1, 15)
        
        # Combine unbroken geometric loop
        ...
        # Transverse Grouser Plates (Rigid steps natively tracing the exact oval)
        t_len = 45 # 45 thick plates distributed evenly across the perimeter
        ...
        for gx, gz in zip(g_x, g_z):
            ...
            # Grouser tooth extrusion normal to surface height
