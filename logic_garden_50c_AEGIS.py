"""
UNE DEEP RESEARCH PROTOCOL v3.1 - SOVEREIGN CODE
SCRIPT: logic_garden_50c_AEGIS.py
MODE:   Industrialist (High-Contrast Daylight Protocol)
TARGET: AEGIS Weapon System (Phased Array Defense)
STYLE:  "The Iron Shield" | 40s Deep Time | 2000x2000 
...
# --- 1. THE DAYLIGHT TACTICAL PALETTE ---
BG_COLOR = "#FFFFFF"        # Daylight Matrix
TEXT_COLOR = "#111115"      # Indestructible Black 
RADAR_GRID = "#94A3B8"      # Passive Grid (Steel)
RADAR_SWEEP = "#005599"     # Active Elements (Deep Marine)
THREAT_RED = "#DE008A"      # Inbound Vampire (Deep Magenta)
FRIENDLY_CYAN = "#00C853"   # Outbound Bird (Jade)
LOCK_YELLOW = "#FFB300"     # FC Lock (Amber)
EXPLOSION = "#FF3300"       # Kinetic Kill (Intense Red)
...
class AegisSim:
    ...
    def update(self, frame_idx):
        ...
        for e in self.threats:
            ...
            if np.sqrt(e.x**2 + e.y**2) < 0.2:
                e.active = False
                self.explosions.append({'x': 0.0, 'y': 0.0, 'life': 60}) # Impact
    ...
    def render(self, frame_idx, ax):
        ...
        # 1. SCOPE GRID
        for r in [2, 4, 6, 8]:
            ax.add_patch(Circle((0,0), r, color=RADAR_GRID, fill=False, linewidth=1.5, alpha=0.5))
        ax.plot([-10, 10], [0, 0], color=RADAR_GRID, linewidth=1.0, alpha=0.5)
        ax.plot([0, 0], [-10, 10], color=RADAR_GRID, linewidth=1.0, alpha=0.5)
        
        # 2. BEAMS (The Phased Array)
        for b in self.beams:
            col = RADAR_SWEEP if b['type'] == 'track' else LOCK_YELLOW
            width = 1.5 if b['type'] == 'track' else 2.5
            alpha = 0.5 if b['type'] == 'track' else 0.8
        ...
        for t in self.threats:
            ...
            ax.add_patch(Polygon(poly, facecolor=THREAT_RED, edgecolor=TEXT_COLOR, lw=1.0))
            # Velocity vector
            ax.plot([t.x, t.x + t.vx*20], [t.y, t.y + t.vy*20], color=THREAT_RED, linewidth=1.5)
            # Designation
            if t.id in self.tracks:
                ax.text(t.x+0.25, t.y+0.25, f"TN-{t.id}", color=THREAT_RED, fontsize=8, weight='bold')
        ...
        for i in self.interceptors:
            ...
            if i.launch_phase < 15:
                ...
                ax.add_patch(Circle((0,0), 0.3, color=EXPLOSION, alpha=0.8))
            else:
                # Missile
                ax.add_patch(Circle((i.x, i.y), 0.1, color=FRIENDLY_CYAN))
                # Trail
                ax.plot([i.x - i.vx*10, i.x], [i.y - i.vy*10, i.y], color=FRIENDLY_CYAN, alpha=0.8, lw=2.0)

        # 5. EXPLOSIONS
        for exp in self.explosions:
            radius = (30 - exp['life']) * 0.05
            ax.add_patch(Circle((exp['x'], exp['y']), radius, color=EXPLOSION, alpha=0.8))
            ax.text(exp['x'], exp['y'], "KILL", color=TEXT_COLOR, fontsize=8, weight='bold', ha='center')

        # 6. SHIP (Center)
        # 0.15 scale ensures visibility on 2k resolution
        ax.add_patch(Circle((0,0), 0.15, color=TEXT_COLOR))
        ...
        ax.text(0, -9.5, status, color=col, ha='center', fontfamily='monospace', fontsize=16, weight='bold',
               bbox=dict(facecolor=BG_COLOR, edgecolor=col, lw=2.0))
               
        ax.text(-9.5, 9.2, "AN/SPY-1 RADAR", color=TEXT_COLOR, fontsize=12, weight='bold', fontfamily='monospace')
        ax.text(-9.5, 8.7, "AUTO-SPECIAL // RANGE: 10 KM", color=RADAR_SWEEP, fontsize=10, weight='bold', fontfamily='monospace')

        ax.set_aspect('equal')
        ax.set_axis_off()
        
        out_dir = "logic_garden_50c_AEGIS_frames"
...
if __name__ == "__main__":
    print(f"[INDUSTRIALIST] Initializing AEGIS Combat System (2000x2000 / Daylight)...")
    
    sim = AegisSim()
    
    for i in range(TOTAL_FRAMES):
        # dpi=200 converts the 10x10 figure into a precise 2000x2000 pixel raster
        ...
        if i % 60 == 0:
            print(f"Absolute Rendering: Frame {i}/{TOTAL_FRAMES}")
            
    print(f"\nMatrix Locked. Geometry preserved at 2K resolution.")
